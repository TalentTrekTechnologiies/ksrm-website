...
const MEDIA_MODULE = 'department_highlights';
const MEDIA_FIELD = 'imageUrl';

@Injectable()
export class DepartmentHighlightsService {
  constructor(
  ...
  async findAllAdmin(departmentId?: number, includeDeleted = false) {
    return this.prisma.departmentHighlight.findMany({
      ...
      orderBy: [{ departmentId: 'asc' }, { kind: 'asc' }, { sortOrder: 'asc' }],
    });
  }

  private async findActiveOrThrow(id: number) {
    const record = await this.prisma.departmentHighlight.findFirst({
      where: { id, deletedAt: null },
    });
    if (!record) {
      throw new NotFoundException(`Department highlight ${id} not found`);
    }
    return record;
  }
  ...
  async create(
    ...
    const created = await this.prisma.departmentHighlight.create({
  ...
  async update(
    id: number,
    dto: UpdateDepartmentHighlightDto,
    admin: RequestAdmin,
    requestId?: string,
  ) {
    const existing = await this.findActiveOrThrow(id);
    const { version, ...rest } = dto;
    assertVersionMatch(existing, version, `Department highlight ${id}`);

    const resolvedUrl = await this.mediaLink.prepareLink(rest.mediaId, 'IMAGE');

    const updated = await this.prisma.departmentHighlight.update({
      where: { id },
      data: {
        ...rest,
  ...
  async restore(id: number, admin: RequestAdmin, requestId?: string) {
    const existing = await this.prisma.departmentHighlight.findFirst({
  ...
  async reorder(
    ...
    const existingRows = await this.prisma.departmentHighlight.findMany({
      where: { id: { in: ids }, deletedAt: null },
      select: { id: true },
    });
    if (existingRows.length !== ids.length) {
      throw new BadRequestException('One or more highlights do not exist');
    }
