...
describe('CampusVideosService', () => {
  let service: CampusVideosService;
  let prisma: {
    campusVideo: {
      findMany: jest.Mock;
      findFirst: jest.Mock;
  ...
  beforeEach(async () => {
    ...
    service = module.get(CampusVideosService);
  });
  ...
  describe('create', () => {
    it('auto-assigns sortOrder to the current count when not provided', async () => {
      prisma.campusVideo.count.mockResolvedValue(2);
      prisma.campusVideo.create.mockResolvedValue({ id: 3, sortOrder: 2 });
      ...
      expect(prisma.campusVideo.create).toHaveBeenCalledWith({
  ...
  describe('update', () => {
    it('409s on stale version', async () => {
      prisma.campusVideo.findFirst.mockResolvedValue({ id: 1, version: 2 });
    ...
    it('404s when the row does not exist or is already soft-deleted', async () => {
      prisma.campusVideo.findFirst.mockResolvedValue(null);
  ...
  describe('softDelete / restore', () => {
    it('soft-deletes by setting deletedAt/deletedBy and logs DELETE', async () => {
      prisma.campusVideo.findFirst.mockResolvedValue({ id: 1, version: 1 });
      prisma.campusVideo.update.mockResolvedValue({
        id: 1,
        deletedAt: new Date(),
        deletedBy: 1,
      });
      ...
      expect(prisma.campusVideo.update).toHaveBeenCalledWith({
        where: { id: 1 },
    ...
    it('restores by clearing deletedAt/deletedBy and logs RESTORE', async () => {
      prisma.campusVideo.findFirst.mockResolvedValue({
        id: 1,
        deletedAt: new Date(),
      });
      prisma.campusVideo.update.mockResolvedValue({ id: 1, deletedAt: null });
  ...
  describe('reorder', () => {
    ...
    it('rejects when an id does not exist', async () => {
      prisma.campusVideo.findMany.mockResolvedValueOnce([{ id: 1 }]); // only 1 of 2 ids found
