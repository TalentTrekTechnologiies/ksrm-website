...
describe('HeroService', () => {
  ...
  describe('create', () => {
    it('creates the singleton hero when none exists yet', async () => {
      ...
      prisma.homepageHero.create.mockResolvedValue({
        id: 1,
        heading: 'Hi',
        version: 1,
      });
      ...
      const result = await service.create(
        { heading: 'Hi', subtitle: 'Sub', videoUrl: '/a.mp4' },
        admin,
        'req-1',
      );
    ...
    it('rejects a second create with 409 once a hero already exists', async () => {
      ...
      await expect(
        service.create(
          { heading: 'x', subtitle: 'y', videoUrl: '/a.mp4' } as any,
          admin,
          undefined,
        ),
  ...
  describe('update', () => {
    ...
    it('updates and increments version when the version matches', async () => {
      prisma.homepageHero.findFirst.mockResolvedValue({
        id: 1,
        version: 1,
        heading: 'old',
      });
      prisma.homepageHero.update.mockResolvedValue({
        id: 1,
        version: 2,
        heading: 'new',
      });
    ...
    describe('Media Library linking', () => {
      ...
      it('on update, re-resolves videoUrl and re-tracks when mediaId changes', async () => {
        ...
        await service.update({ mediaId: 9, version: 1 }, admin, undefined);
      ...
      it('on update, unlinks and untracks when mediaId is explicitly set to null, without touching videoUrl', async () => {
        ...
        await service.update({ mediaId: null, version: 1 }, admin, undefined);
        ...
        expect(prisma.homepageHero.update).toHaveBeenCalledWith(
          expect.objectContaining({
            data: expect.objectContaining({ mediaId: null }),
          }),
        );
        ...
        expect(prisma.homepageHero.update).toHaveBeenCalledWith(
          expect.objectContaining({
            data: expect.not.objectContaining({ videoUrl: expect.anything() }),
          }),
        );
      });
    });
